...
router.get("/vehicles/:datumOd/:datumDo/:siflok", async function(req, res, next) {
    ...
    db.any("SELECT DISTINCT ON (model.sifmodel) model, vozilo.sifvozilo, registratskaoznaka, nazivproizvodac, nazivmodel, nazivvrstamodel, nazivvrstamotor, nazivvrstamjenjac, potrosnja, urlslika, cijenapodanu FROM najam RIGHT JOIN vozilo ON vozilo.sifvozilo = najam.sifvozilo JOIN lokacija_vozilo ON vozilo.sifvozilo = lokacija_vozilo.sifvozilo NATURAL JOIN model NATURAL JOIN proizvodac NATURAL JOIN vrsta_model NATURAL JOIN vrsta_mjenjac NATURAL JOIN vrsta_motor NATURAL JOIN cjenik WHERE vozilo.aktivno=TRUE AND vozilo.sifvozilo NOT IN (SELECT sifvozilo FROM najam WHERE planiranidatumvrijemeod < $1 AND planiranidatumvrijemedo > $1) AND vozilo.sifvozilo NOT IN (SELECT sifvozilo FROM najam WHERE planiranidatumvrijemeod > $1 AND planiranidatumvrijemedo < $2) AND vozilo.sifvozilo NOT IN (SELECT sifvozilo FROM najam WHERE planiranidatumvrijemeod < $2 AND planiranidatumvrijemedo > $2) AND (CASE WHEN vozilo.sifvozilo IN (SELECT DISTINCT najam.sifvozilo FROM najam) THEN siflokvracanja = $3 ELSE lokacija_vozilo.siflokacija = $3 AND sifstatus = 2 END) AND ($1::date <@ period)",
